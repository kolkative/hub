...
(function () {
  ...
  window.addEventListener("load", function () {
    if (!document.getElementById("x-header")) {
      ...
      header.id = "x-header";
      header.innerHTML =
        '<img src="https://i.imgur.com/uyJEQnp.png" alt="Kolkative Hub" class="x-header-logo"/><span class="x-header-title">Kolkative Hub</span>';
      document.body.insertBefore(header, document.body.firstChild);
    }
    // Pastikan x-toggle selalu di atas header
    var xToggle = document.getElementById("x-toggle");
    if (xToggle) document.body.appendChild(xToggle);
  });
})();
